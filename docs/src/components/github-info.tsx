...
export function GithubInfo({ repo, owner, token, baseUrl, fetchOptions, locale, ...props }: GithubInfoProps) {
    ...
    return (
        <a
            ...
            {...props}
            className={cn(
                "relative flex flex-col gap-0.5 px-3 rounded-lg overflow-hidden w-full",
                "border border-fd-border bg-fd-secondary/40",
                "text-sm text-fd-muted-foreground transition-colors",
                "hover:bg-fd-secondary/60 hover:text-fd-foreground",
                props.className,
            )}
        >
            <p className="text-fd-foreground flex items-center gap-2 truncate leading-none font-medium">
                <svg fill="currentColor" viewBox="0 0 24 24" className="size-3.5 shrink-0">
                    <title>GitHub</title>
            ...
            <div className="mb-3 text-fd-muted-foreground flex items-center gap-3 text-xs leading-none">
